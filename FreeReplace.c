...
{
  int flush2;
  char commandstr[10],taskstring[50],optionstring[50];
  for(flush2=0;flush2<sizeof(optionstring);flush2++)
  {
    optionstring[flush2]=' ';
    optionstring[flush2]=' ';
  }
  printf("\n\tEnter your taskstring to process : \t");
  gets(taskstring);
  printf("\n\tEnter the replace options here : \t");
  gets(optionstring);
